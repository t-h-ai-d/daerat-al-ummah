...
function Saved() {
  ...
  return <div dir="rtl" className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><PageIntro eyebrow="مساحتك الهادئة" title="المحفوظات" body="منشورات اخترت الاحتفاظ بها للقراءة أو المراجعة. مجموعاتك خاصة بك دائماً، ولا تُرتَّب وفق الإعجابات." />
    <section className="mb-6 rounded-[22px] border border-[#dce5da] bg-white p-4 shadow-[0_8px_24px_rgba(27,66,49,0.035)] dark:border-[#3d5148] dark:bg-[#15231f]"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold tracking-[0.14em] text-[#8a9c92]">تنظيم خاص</p><h2 className="mt-1 text-sm font-extrabold text-[#264a3e] dark:text-[#e5f0e8]">مجموعات المحفوظات</h2></div><form onSubmit={event => { event.preventDefault(); if (collectionTitle.trim()) createCollection.mutate({ title: collectionTitle.trim() }); }} className="flex min-w-[230px] flex-1 gap-2 sm:max-w-sm"><Input value={collectionTitle} onChange={event => setCollectionTitle(event.target.value)} maxLength={80} placeholder="مثل: دروس أعود إليها" className="h-9 rounded-xl border-[#d9e4d8] text-xs dark:border-[#4a5e53]" /><Button type="submit" disabled={createCollection.isPending || !collectionTitle.trim()} className="h-9 shrink-0 rounded-xl bg-[#0d4937] px-3 text-[11px] hover:bg-[#176047]">إنشاء</Button></form></div>
