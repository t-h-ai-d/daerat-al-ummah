...
function Explore() {
  ...
  return <div dir="rtl" className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    ...
    {isAuthenticated && <section className="mt-8 rounded-[20px] border border-[#e5e0d2] bg-[#fffdf5] p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold tracking-[0.14em] text-[#8b7540]">خصوصيتك</p><h2 className="mt-1 text-sm font-extrabold text-[#40594d]">الأعضاء المحظورون</h2></div><ShieldCheck className="text-[#9a7b2c]" size={19} /></div>{blocked.isLoading ? <p className="mt-3 text-xs text-[#82968b]">يجري تجهيز القائمة…</p> : blocked.data?.length ? <div className="mt-3 flex flex-wrap gap-2">{blocked.data.map(person => <div key={person.id} className="flex items-center gap-2 rounded-xl border border-[#eadfc5] bg-white px-3 py-2"><span className="max-w-[150px] truncate text-xs font-bold text-[#526b5e]">{person.name || person.username || "عضو"}</span><Button type="button" variant="outline" onClick={() => unblock.mutate({ blockedId: person.id })} disabled={unblock.isPending} className="h-7 rounded-lg border-[#eadfc5] px-2 text-[10px] text-[#806c38]">إلغاء الحظر</Button></div>)}</div> : <p className="mt-3 text-xs text-[#82968b]">لا توجد حسابات محظورة.</p>}</section>}
