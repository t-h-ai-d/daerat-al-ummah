...
type PageKind = "explore" | "saved" | "playlists" | "notifications" | "rules" | "profile";
...
function Saved({ playlistMode = false }: { playlistMode?: boolean }) {
  const { isAuthenticated } = useAuth();
  ...
  const collectionPosts = trpc.social.savedCollectionPosts.useQuery({ collectionId: selectedCollectionId ?? 0 }, { enabled: isAuthenticated && selectedCollectionId !== null });
  const utils = trpc.useUtils();
  const invalidateSaved = () => { void utils.social.savedPosts.invalidate(); void utils.social.savedCollections.invalidate(); void utils.social.savedCollectionPosts.invalidate(); };
  const removeSaved = trpc.social.toggleSavedPost.useMutation({ onSuccess: () => { invalidateSaved(); toast.success("أُزيلَ المنشور من المحفوظات ومجموعاته الخاصة."); }, onError: error => toast.error(error.message) });
  const createCollection = trpc.social.createSavedCollection.useMutation({ onSuccess: collection => { setCollectionTitle(""); setSelectedCollectionId(collection.id); invalidateSaved(); toast.success("أُنشئت مجموعة محفوظات خاصة."); }, onError: error => toast.error(error.message) });
  const deleteCollection = trpc.social.deleteSavedCollection.useMutation({ onSuccess: () => { setSelectedCollectionId(null); invalidateSaved(); toast.success("حُذفت المجموعة فقط؛ بقيت المحفوظات العامة كما هي."); }, onError: error => toast.error(error.message) });
  const addToCollection = trpc.social.addPostToSavedCollection.useMutation({ onSuccess: () => { invalidateSaved(); toast.success("أُضيف المنشور إلى المجموعة."); }, onError: error => toast.error(error.message) });
  const removeFromCollection = trpc.social.removePostFromSavedCollection.useMutation({ onSuccess: () => { invalidateSaved(); toast.success("أُزيل المنشور من هذه المجموعة فقط."); }, onError: error => toast.error(error.message) });
  const activeCollection = collections.data?.find(collection => collection.id === selectedCollectionId);
  const displayedPosts = selectedCollectionId === null ? saved.data : collectionPosts.data;
  const sectionTitle = playlistMode ? "قوائم التشغيل" : "المحفوظات";
  const pageBody = playlistMode ? "قوائم خاصة تجمع ما اخترت مشاهدته لاحقاً أو الرجوع إليه، ولا يراها غيرك." : "منشورات اخترت الاحتفاظ بها للقراءة أو المراجعة. مجموعاتك خاصة بك دائماً، ولا تُرتَّب وفق الإعجابات.";
  if (!isAuthenticated) return <div dir="rtl" className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><PageIntro eyebrow="مكتبتك الخاصة" title={sectionTitle} body={playlistMode ? "أنشئ قوائم مشاهدة خاصة من المواد التي تحفظها، من دون ترتيب قهري أو اقتراحات لا تنتهي." : "احفظ ما تريد الرجوع إليه لاحقاً، من دون أن تتحول الصفحة إلى تمرير لا ينتهي."} /><div className="rounded-[24px] border border-dashed border-[#cbd9cd] bg-[#fbfcf8] px-6 py-16 text-center"><Bookmark className="mx-auto text-[#25684d]" size={28} /><h2 className="mt-5 font-display text-xl font-semibold text-[#264b3e]">سجّل الدخول لرؤية {playlistMode ? "قوائم التشغيل" : "محفوظاتك"}.</h2><Button onClick={() => { window.location.href = "/auth"; }} className="mt-5 rounded-xl bg-[#0d4937] text-xs hover:bg-[#176047]">انضم إلى الدائرة</Button></div></div>;
  return <div dir="rtl" className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><PageIntro eyebrow="مكتبتك الخاصة" title={sectionTitle} body={pageBody} />
    <section className="mb-6 rounded-[22px] border border-[#dce5da] bg-white p-4 shadow-[0_8px_24px_rgba(27,66,49,0.035)] dark:border-[#3d5148] dark:bg-[#15231f]"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold tracking-[0.14em] text-[#8a9c92]">تنظيم خاص</p><h2 className="mt-1 text-sm font-extrabold text-[#264a3e] dark:text-[#e5f0e8]">{playlistMode ? "قوائمك الخاصة" : "مجموعات المحفوظات"}</h2></div><form onSubmit={event => { event.preventDefault(); if (collectionTitle.trim()) createCollection.mutate({ title: collectionTitle.trim() }); }} className="flex min-w-[230px] flex-1 gap-2 sm:max-w-sm"><Input value={collectionTitle} onChange={event => setCollectionTitle(event.target.value)} maxLength={80} placeholder={playlistMode ? "مثل: دروس أتعلّم منها" : "مثل: دروس أعود إليها"} className="h-9 rounded-xl border-[#d9e4d8] text-xs dark:border-[#4a5e53]" /><Button type="submit" disabled={createCollection.isPending || !collectionTitle.trim()} className="h-9 shrink-0 rounded-xl bg-[#0d4937] px-3 text-[11px] hover:bg-[#176047]">إنشاء</Button></form></div>
      <div className="mt-4 flex flex-wrap gap-2"><Button type="button" variant={selectedCollectionId === null ? "default" : "outline"} onClick={() => setSelectedCollectionId(null)} className={selectedCollectionId === null ? "h-8 rounded-lg bg-[#0d4937] px-3 text-[11px] hover:bg-[#176047]" : "h-8 rounded-lg border-[#dce5da] px-3 text-[11px] text-[#557064] dark:border-[#4a5e53] dark:text-[#d5e4da]"}>كل المحفوظات ({saved.data?.length ?? 0})</Button>{collections.data?.map(collection => <Button key={collection.id} type="button" variant={selectedCollectionId === collection.id ? "default" : "outline"} onClick={() => setSelectedCollectionId(collection.id)} className={selectedCollectionId === collection.id ? "h-8 rounded-lg bg-[#1a6147] px-3 text-[11px] hover:bg-[#176047]" : "h-8 rounded-lg border-[#dce5da] px-3 text-[11px] text-[#557064] dark:border-[#4a5e53] dark:text-[#d5e4da]"}>{collection.title} ({collection.itemCount})</Button>)}</div>
      {activeCollection && <div className="mt-4 flex items-center justify-between gap-3 rounded-xl bg-[#f5f8f2] px-3 py-2 dark:bg-[#1c2c26]"><p className="text-xs text-[#60786c] dark:text-[#c7d7ce]">هذه المجموعة خاصة ولا يراها أي عضو آخر.</p><Button type="button" variant="outline" onClick={() => { if (window.confirm(`حذف مجموعة «${activeCollection.title}»؟ لن تُحذف المنشورات المحفوظة.`)) deleteCollection.mutate({ collectionId: activeCollection.id }); }} disabled={deleteCollection.isPending} className="h-7 rounded-lg border-[#e5d4cc] px-2 text-[10px] text-[#8a584a] dark:border-[#764d43] dark:text-[#e4b1a4]">حذف المجموعة</Button></div>}</section>
    {(saved.isLoading || (selectedCollectionId !== null && collectionPosts.isLoading)) ? <div className="flex items-center gap-2 rounded-[20px] border border-[#dce5da] bg-white p-6 text-sm text-[#6a8377]"><Loader2 className="animate-spin" size={17} />يجري تجهيز المحفوظات…</div> : displayedPosts?.length ? <div className="space-y-4">{displayedPosts.map(post => <article key={post.id} className="rounded-[22px] border border-[#dce5da] bg-white p-5 shadow-[0_8px_24px_rgba(27,66,49,0.035)] dark:border-[#3d5148] dark:bg-[#15231f]"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-extrabold text-[#264a3e] dark:text-[#e5f0e8]">{post.author.name || post.author.username || "عضو في الدائرة"}</p><p className="mt-1 text-[11px] text-[#81958a]">حُفِظَ في {new Date(post.savedAt).toLocaleDateString("ar")}</p></div><div className="flex flex-wrap justify-end gap-2"><Bookmark className="mt-1 text-[#b58d32]" size={18} />{selectedCollectionId !== null && <Button type="button" variant="outline" onClick={() => removeFromCollection.mutate({ collectionId: selectedCollectionId, postId: post.id })} disabled={removeFromCollection.isPending} className="h-8 rounded-lg border-[#dce5da] px-2.5 text-[11px] text-[#617b6e] dark:border-[#4a5e53] dark:text-[#d5e4da]">إزالة من المجموعة</Button>}<Button type="button" variant="outline" onClick={() => removeSaved.mutate({ postId: post.id })} disabled={removeSaved.isPending} className="h-8 rounded-lg border-[#dce5da] px-2.5 text-[11px] text-[#617b6e] dark:border-[#4a5e53] dark:text-[#d5e4da]">إزالة الحفظ</Button></div></div>{selectedCollectionId === null && collections.data?.length ? <label className="mt-4 block text-[11px] font-bold text-[#6a8377] dark:text-[#c5d7cc]">أضفه إلى مجموعة خاصة<select defaultValue="" onChange={event => { const collectionId = Number(event.target.value); if (collectionId) { addToCollection.mutate({ collectionId, postId: post.id }); event.currentTarget.value = ""; } }} disabled={addToCollection.isPending} className="mr-2 h-8 rounded-lg border border-[#dce5da] bg-white px-2 text-[11px] text-[#466459] dark:border-[#4a5e53] dark:bg-[#1c2c26] dark:text-[#d5e4da]"><option value="">اختر مجموعة</option>{collections.data.map(collection => <option key={collection.id} value={collection.id}>{collection.title}</option>)}</select></label> : null}{post.title && <h2 className="mt-4 font-display text-lg font-bold text-[#24483d] dark:text-[#e5f0e8]">{post.title}</h2>}<p dir="auto" className="mt-3 whitespace-pre-wrap text-sm leading-7 text-[#3d5e51] dark:text-[#c7d7ce]">{post.content || "مَنْشُورٌ بِمُرْفَقٍ فقط."}</p></article>)}</div> : <div className="rounded-[24px] border border-dashed border-[#cbd9cd] bg-[#fbfcf8] px-6 py-16 text-center dark:border-[#465a50] dark:bg-[#15231f]"><Bookmark className="mx-auto text-[#25684d]" size={28} /><h2 className="mt-5 font-display text-xl font-semibold text-[#264b3e] dark:text-[#e5f0e8]">{selectedCollectionId === null ? "لا توجد محفوظات بعد." : "لا توجد منشورات في هذه المجموعة."}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#778d82] dark:text-[#bdcec4]">{selectedCollectionId === null ? "استخدم زر الحفظ في المنشور عندما تجد شيئاً نافعاً تريد العودة إليه." : "ارجع إلى كل المحفوظات وأضف منشورات نافعة إلى هذه المجموعة."}</p></div>}</div>;
}
...
export default function PlatformPage({ kind }: { kind: PageKind }) {
  ...
  const page = kind === "explore" ? <Explore /> : kind === "saved" ? <Saved /> : kind === "playlists" ? <Saved playlistMode /> : kind === "notifications" ? <Notifications /> : kind === "profile" ? <><Profile /><FriendPrivacyPanel /></> : <Rules />;
