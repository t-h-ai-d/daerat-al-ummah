...
import { ImagePlus, Loader2, MessageCircleMore, Send, UserPlus, X } from "lucide-react";
...
function readFile(file: File) {
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error("تعذّرت قراءة ملف GIF."));
...
export default function ChatPage() {
  ...
  const [gif, setGif] = useState<{ url: string; filename: string } | null>(null);
  ...
  const start = trpc.chat.start.useMutation({ onSuccess: async result => { setActiveConversationId(result.conversationId); setUsername(""); await utils.chat.conversations.invalidate(); }, onError: error => toast.error(error.message) });
  const uploadGif = trpc.social.uploadAttachment.useMutation({ onError: error => toast.error(error.message) });
  const send = trpc.chat.send.useMutation({ onSuccess: async () => { setDraft(""); setGif(null); await Promise.all([utils.chat.messages.invalidate(), utils.chat.conversations.invalidate()]); }, onError: error => toast.error(error.message) });
