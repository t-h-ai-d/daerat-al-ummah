...
export default function CreatorStudio() {
  ...
  return <section className="mt-6 max-w-4xl px-4 pb-12 sm:px-6 lg:px-8" dir="rtl">
    <div className="rounded-[24px] border border-[#dce5da] bg-white p-5 shadow-[0_14px_40px_rgba(26,69,52,0.05)] sm:p-6">
      ...
      <div className="mt-8 rounded-2xl border border-[#ecd1cc] bg-[#fff8f6] p-4"><div className="flex items-start gap-3"><UserRoundX className="mt-0.5 text-[#ad574a]" size={19} /><div><h3 className="text-sm font-extrabold text-[#7e352d]">بدء حساب جديد</h3><p className="mt-1 text-xs leading-5 text-[#92635b]">يحذف هذا الحساب وكل بياناته من أمّة سيركل، ثم يمكنك التسجيل مجددًا بنفس البريد واسم المستخدم.</p><Button type="button" size="sm" variant="outline" disabled={deleteAccount.isPending} onClick={() => { if (window.confirm("سيُحذف حسابك ومنشوراتك ورسائلك وبياناتك نهائيًا. هل تريد المتابعة؟")) deleteAccount.mutate(); }} className="mt-3 rounded-lg border-[#dfb1aa] text-[#9d4338] hover:bg-[#fff1ee]"><Trash2 size={14} />حذف حسابي وبدء جديد</Button></div></div></div>
    </div>
  </section>;
}
